...
let lastCar = 0;
console.log(
  `The last car in the inventory is a ${inventory[inventory.length - 1].car_make} ${inventory[inventory.length - 1].car_model}.`
);
...
let oldCars = carYears.filter(function(value) {
  return value < 2000;
});
...
let BMWAndAudi = inventory.filter(function(value) {
  return value.car_make == 'BMW' || value.car_make == 'Audi';
});
